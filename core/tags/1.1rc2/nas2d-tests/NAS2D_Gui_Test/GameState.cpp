...
void GameState::initialize()
{
	...
	//Window mWindow("Test Window", 50, 50, 125, 125);
	mGui->add(new Window("Test Window", 50, 50, 125, 125));
	//mWindow.drawBorder(true);

	Singleton<EventHandler>::get().event(SDL_MOUSEMOTION).connect(this, &GameState::onMouseMotion);
	Singleton<EventHandler>::get().event(SDL_MOUSEBUTTONUP).connect(this, &GameState::onMouseButtonUp);
	Singleton<EventHandler>::get().event(SDL_MOUSEBUTTONDOWN).connect(this, &GameState::onMouseButtonDown);
}
