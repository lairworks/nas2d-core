...
{
	auto valueAsStr = [this, &key]() -> std::string {
		try
		{
	...
	{
		const auto valueAsBool = [&valueAsStr]() {
			try
			{
			...
			{
				const std::string err_msg{"Dictionary value \'" + valueAsStr + "\' is not convertible to 'integer'"};
				throw std::runtime_error(err_msg);
			}
			catch (const std::out_of_range&)
			{
				const std::string err_msg{"Dictionary value \'" + valueAsStr + "\' is out of range of 'integer'."};
				throw std::runtime_error(err_msg);
			}
		}(); // IILE
		return valueAsBool;
...
{
	const auto valueAsStr = [this, &key]() -> std::string {
		try
		{
			return value<std::string>(key);
		}
...
unsigned long NAS2D::Dictionary::value_helper(const std::string& key, unsigned long) const
{
	...
	const auto valueAsULong = [&valueAsStr]() {
		try
		{
...
long long NAS2D::Dictionary::value_helper(const std::string& key, long long) const
{
	...
	const auto valueAsLongLong = [&valueAsStr]() {
		try
		{
