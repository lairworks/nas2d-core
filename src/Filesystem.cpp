...
void Filesystem::init(const std::string& argv_0, const std::string& appName, const std::string& organizationName, const std::string& dataPath)
{
	...
	if(PHYSFS_isInit())
	{
		throw filesystem_already_initialized();
	}
	...
	{
		std::cout << std::endl
				  << "(FSYS) Error setting sane config. " << getLastPhysfsError() << "." << std::endl;
	}
	...
	{
		std::cout << std::endl
				  << "(FSYS) Couldn't find data path '" << mDataPath << "'. " << getLastPhysfsError() << "." << std::endl;
	}
#endif
...
bool Filesystem::mount(const std::string& path) const
{
	...
	if(!PHYSFS_isInit())
	{
		throw filesystem_not_initialized();
	}
